...
def make_grid_auto(im, grid):
    ...
    def find_grid_positions_1d(image, axis, n):
        ...
        #### the following block basically aligns a perfect grid with our colony array
        ### it so by comparing a perfect sinusoid with imvals;
        ## it gradually shifts by 1pxl imvals and compares it to a perfect sinusoid along the axis and find the shift
        # with the minimal distance. It then redefines 0 
...
def quantify_single_image_HSV(orig_image, grid, auto, t, d, s, negate, RGB_filter, HSV_filter, reportAll=False, hardImageThreshold=None, hardSizeThreshold=None, localThresh=None):
    ...
    image_HSV = check_and_negate(image_HSV, negate=0)
...
def Filter_Z(DF, metrics, Z_tresh = Z_thresh):
    ...
    for metric in tqdm(metrics):
      ...
      #  print(abs(DF[metric+'_Z']))
        #np.abs added on the 26.10.22
        colony2nan = np.abs(DF[metric+'_Z']) > Z_tresh #abs()
...
def Velber_Preprocess(im, crop=[70,70,80,80]): #crop adjusts cropping of final image T,B,L,R
    ...
    #cropping:
    top = int(TL[1]+crop[0])
    bottom = int(BR[1]-crop[1])
    left = int(TL[0]+crop[2])
    right = int(BR[0]-crop[3])
    im = im[top:bottom,left:right]
    return im,[angle,top,bottom,left,right]

def scale_it_all(DF, metrics ,default_cols=['gene','plate','row','column'], p_format=[32,48] ):
    
    '''this function sets equal: 
    the plates medians to the screen median
    the rows medians to the screen median
    the columns medians to the screen median'''
    #Subset data set for ease:
    cols= default_cols
    [cols.append(m ) for m in metrics]
    Data_corrected = DF[cols]
    for metric in metrics: #['mean_intensity.24.YUGN','area.24.YUGN']:
        ...
        screen_median = Data_corrected[metric].median()
    #create empty column:
        Data_corrected['corrected_' + metric]=  Data_corrected['plate'].copy()*np.nan
        for p in tqdm( np.unique(Data_corrected.plate)):
            i_select= (Data_corrected.plate == p) & (Data_corrected[metric].isna() ==0)
            scale = screen_median/Data_corrected[i_select][metric].median()
            Data_corrected.loc[i_select,'corrected_' + metric]=  Data_corrected.loc[i_select,metric]*scale
            
            #equalize individual rows to screen median
            for row in range(p_format[0]):
                i_select = (Data_corrected.plate == p) & (Data_corrected.row == row) & (Data_corrected[metric].isna() ==0)

                
                scale = screen_median/Data_corrected[i_select]['corrected_'+metric].median()
                Data_corrected.loc[i_select,'corrected_' + metric]=  Data_corrected.loc[i_select, 'corrected_' +metric]*scale
                #print( Data_corrected.loc[i_select,'corrected_' +metric]*scale)
            #equalize individual columns to screen median
            for column in range(p_format[1]):
                i_select = (Data_corrected.plate == p) & (Data_corrected.column == column) & (Data_corrected[metric].isna() ==0)
                scale = screen_median/Data_corrected[i_select]['corrected_' +metric].median()
                Data_corrected.loc[i_select,'corrected_' + metric]=  Data_corrected.loc[i_select,'corrected_' +metric]*scale
